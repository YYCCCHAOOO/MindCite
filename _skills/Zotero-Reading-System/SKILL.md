...
```text
请帮我只读更新 Zotero 索引，然后继续精读下一批 2 篇有 PDF 或全文缓存的文献。不要写回 Zotero。完成后告诉我生成了哪些 notes、哪些条目缺 PDF、日志文件在哪里。
```

如果只是测试通路：

```text
请设置 MINDCITE_OFFLINE=1，离线测试精读 1 篇文献，不调用远程模型。
```

Codex 应该先解释将要做的事，再执行脚本，并用自然语言总结结果，不要求用户理解脚本参数。
