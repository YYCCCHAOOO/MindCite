...
ResearchVault 采用“索引层 + 笔记层 + 治理层”的结构。
...
- `Xiaohongshu-Knowledge-Translation`：把审核后的研究内容转成公开表达。

## 配置层

所有脚本复用 `_skills/common/researchvault_config.py`。配置优先级：

1. 系统环境变量。
2. `.env`。
3. `config/researchvault.json`。
4. `config/researchvault.example.json`。
5. 当前仓库根目录默认值。
